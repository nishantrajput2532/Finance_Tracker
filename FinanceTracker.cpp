...
bool FinanceTracker::deleteExpense(size_t oneBasedIndex) {
    // Bounds check: reject 0 (there is no "expense #0") and anything past
    // the end of the list. This is what makes deletion crash-proof against
    // a user typing a wrong or out-of-range number.
    if (oneBasedIndex == 0 || oneBasedIndex > expenses_.size()) {
        return false;
    }

    // Convert the 1-based index the user sees to a 0-based vector index,
    // then std::vector::erase removes that single element. This is O(n)
    // because everything after it shifts left by one - perfectly fine for
    // a personal-scale dataset of a few hundred/thousand expenses.
    expenses_.erase(expenses_.begin() + static_cast<long>(oneBasedIndex - 1));

    saveToFile(); // Persist immediately so the deletion isn't lost on exit
    return true;
}
